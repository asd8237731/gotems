...
func New(cfg OrchestratorConfig, logger *slog.Logger) *Orchestrator {
	...
	return &Orchestrator{
		...
		router:      router,
		dag:         NewDAGExecutor(router, logger),
...
func (o *Orchestrator) RunWithTasks(ctx context.Context, tasks []*task.Task) (*schema.FinalResult, error) {
	...
	for i, layer := range dag.Layers() {
		o.logger.Info("DAG layer", "index", i, "tasks", layer)
	}
	...
	for _, r := range results {
		_ = o.costTracker.Track(cost.Record{
			AgentID:   r.AgentID,
			Provider:  r.Provider,
			TokensIn:  r.TokensIn,
...
func (o *Orchestrator) runSingle(ctx context.Context, t *task.Task) (*schema.FinalResult, error) {
	...
	// 熔断检查
	if !o.breaker.Allow(provider) {
		return nil, fmt.Errorf("provider %s is circuit-broken, try later", provider)
	}

	// 限流等待
	if err := o.limiter.Wait(ctx, provider); err != nil {
	...
	if err != nil {
		o.breaker.RecordFailure(provider)
		return nil, fmt.Errorf("execute task: %w", err)
	}

	o.breaker.RecordSuccess(provider)
...
// runConsensus 竞赛模式：同一任务发给所有 Agent
func (o *Orchestrator) runConsensus(ctx context.Context, t *task.Task) (*schema.FinalResult, error) {
	...
	results, err := ParallelExecute(ctx, agents, t)
	if err != nil {
